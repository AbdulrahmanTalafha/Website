        ...
        <div className="space-y-5">
          {rows.map((row, rowIndex) => (
            <div
              ...
              className={`grid grid-cols-1 lg:grid-cols-5 gap-5 items-stretch ${isRTL ? 'lg:flex lg:flex-row-reverse lg:items-stretch' : ''}`}
            >
              <Link
                href={`/${locale}/publications-reports/${row.featured.slug}`}
                className={`lg:col-span-3 lg:flex-[3] group relative rounded-2xl overflow-hidden shadow-xl min-h-[320px] lg:min-h-0 lg:h-full flex flex-col bg-primary-500 ${row.side.length === 0 ? 'lg:col-span-5' : ''}`}
              >
                <Image
                  src={row.featured.coverImage}
                  ...
                  <div className={`flex items-center justify-between ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <div className={`flex items-center gap-3 text-white/50 text-xs ${isRTL ? 'flex-row-reverse' : ''}`}>
                      <span className="flex items-center gap-1">
                        ...
                        {row.featured.publishDate.slice(0, 7)}
                      </span>
                      {row.featured.pages && (
                        <span className="flex items-center gap-1">
                          <FileText className="w-3.5 h-3.5" />
                          {row.featured.pages} {locale === 'ar' ? 'صفحة' : 'pages'}
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-3">
                      <button
                        type="button"
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); window.open(row.featured.pdfUrl, '_blank'); }}
              ...
              {row.side.length > 0 && (
              <div className="lg:col-span-2 lg:flex-[2] flex flex-col gap-5 lg:h-full">
                {row.side.map((pub) => (
                  <Link
                    key={pub.id}
